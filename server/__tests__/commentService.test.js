...
describe('comment ownership', () => {
  test('a comment can be created and listed for its article', () => {
    const { body } = createComment('article-1', { id: userA.id, full_name: userA.fullName }, 'first comment');
    const list = listComments('article-1');
    expect(list.body.comments.some((c) => c.id === body.comment.id)).toBe(true);
  });

  test('updateComment/deleteComment are only reachable for the row that was actually loaded by ownership middleware', () => {
    ...
    // authorized.
    const { body } = createComment('article-2', { id: userA.id, full_name: userA.fullName }, 'owned comment');
    const commentId = body.comment.id;

    const updated = updateComment(commentId, 'edited body');
    expect(updated.body.comment.edited).toBe(true);
    expect(updated.body.comment.body).toBe('edited body');

    deleteComment(commentId);
    expect(findCommentById(commentId)).toBeUndefined();
  });
});

describe('account deletion anonymizes comments instead of removing them', () => {
  test('comments survive with "Deleted User" attribution and a null user id', () => {
    const { body } = createComment('article-3', { id: userB.id, full_name: userB.fullName }, 'before deletion');
    anonymizeUserComments(userB.id);

    const comment = findCommentById(body.comment.id);
